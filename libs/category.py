...
from libs.utils import get_url, encode, get_kodi_version
...
def list_series(label, id, page):
    ...
    if data is None:
        ...
        if int(offset) > 0:
            ...
            url = get_url(action='list_series', label = encode(label), id = id, page = int(page) - 1)  
...
def get_show_listitem(label, id):
    ...
    if data['showType'] in ['series', 'magazine']:
        url = get_url(action='list_series', label = label + '/' + data['title'], id = idec, page = 1)  
    else:
...
def list_category(label, categoryId, subcategory, page):
    ...
    if data is None:
        ...
        if int(offset) > 0:
            list_item = xbmcgui.ListItem(label='Předchozí strana (' + str(int(page) - 1) + '/' + str(math.ceil(totalCount/pagesize)) + ')')
            url = get_url(action='list_category', label = encode(label), categoryId = categoryId, subcategory = subcategory, page = int(page) - 1)  
            list_item.setArt({ 'thumb' : os.path.join(icons_dir , 'previous_arrow.png'), 'icon' : os.path.join(icons_dir , 'previous_arrow.png') })
            xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
